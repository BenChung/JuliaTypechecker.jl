...
module asm
	...
	#tc: @assert length(h) == (prb.nx+prb.nu)*(prb.N-1) "Length of h is incorrect."
	#tc: @assert length(g) == 2*prb.nu*(prb.N-2)+prb.nx*(prb.N-1) "Length of g is incorrect."
	...
	function set_constraint_jump_vectorized!(model,zz)
		...
		#tc: @constraint(model,[j=1:N-1],-prb.umax .≤ zz[(j-1)*nxnu+1:(j-1)*nxnu+nu] .≤  prb.umax)
		#tc: @constraint(model,[j=1:N-1],-prb.pmax .≤ zz[nu+(j-1)*nxnu+1:nu+(j-1)*nxnu+nxb2] .≤  prb.pmax)
		#tc: @constraint(model,[j=1:N-1],-prb.vmax .≤ zz[nu+(j-1)*nxnu+nxb2+1:nu+(j-1)*nxnu+nx] .≤  prb.vmax)
	end
	...
	function set_constraint_jump_decomposed!(model,x,u)
		#tc: @constraint(model,[t=1:N-1], -prb.pmax .≤ x[1:nxb2,t+1] .≤ prb.pmax)
		#tc: @constraint(model,[t=1:N-1], -prb.vmax .≤ x[nxb2+1:nx,t+1] .≤ prb.vmax)
		#tc: @constraint(model,[t=1:N-1], -prb.umax .≤ u[1:nu,t] .≤ prb.umax)
		#tc: @constraint(model,[t=1:N-2],-prb.dumax .≤ u[:,t+1] .- u[:,t] .≤ prb.dumax)
	end
